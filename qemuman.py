...
class Manager:
    ...
    def start_vm(self):
        try:
            ...
            self.qemu_process = Popen(
                f"{self.qemu_type_box.get()} -cdrom {self.cdrom_path.get()} {'-sdl' if self.qemu_sdl_window.get() else ''}",
                stdout=PIPE,
                stderr=PIPE,
            )
    ...
    def create_menu_items(self):
        ...
        self.menubar.add_cascade(label ='Virtual Machine', menu = virtual_machine)
        ...
        about.add_command(label ='About', command=lambda: alert("About", "QEMU Manager is a simple GUI frontend for QEMU written in TKinter and Python.\n\nCreated by yeppiidev"))
    ...
    def create_widgets(self):
        ...
        self.start_vm_btn = ttk.Button(self.root, text="Start VM", command=self.start_vm)
        self.start_vm_btn.pack(ipadx=10, ipady=10, padx=10, pady=10)
        self.start_vm_btn.place(x=490, y=445)

        # Add a button to kill the VM
        self.kill_vm_btn = ttk.Button(self.root, text="Terminate QEMU", command=self.kill_vm)
        ...
        self.hdd_path = ttk.Entry(self.root, textvariable=self.hdd_path_text)
        self.hdd_path.pack(fill="x", padx=15)
        self.hdd_path.focus()
